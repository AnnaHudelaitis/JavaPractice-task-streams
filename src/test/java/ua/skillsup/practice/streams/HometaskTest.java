...
public class HometaskTest {
	...
	public void shouldCountSumOfNumbersInTheList() throws Exception {
		...
		long quantity = listOfNumbers.stream().mapToInt(Integer::intValue).sum();
	...
	public void shouldFilterAllNumbersGreaterThan5AndDividedBy2() throws Exception {
		...
		List<Integer> numbersGreaterThan5 = numbers.stream().filter(i -> i>5).collect(Collectors.toList());
		List<Integer> numbersGreaterThan5AndDividedBy2 = numbersGreaterThan5.stream().filter(i -> i%2 ==0).collect(Collectors.toList());

		assertThat(numbersGreaterThan5AndDividedBy2).contains(6, 8);
	}
...
//
		List<Integer> MultiplyNumbers = numbers.stream().map (i -> i*2).collect(Collectors.toList());
		List<String> multipliedNumbersAsString = MultiplyNumbers.stream().map(i -> i.toString()).collect(Collectors.toList());
	...
	public void shouldCheckIfThereIsANumberGreaterThan4() throws Exception {
		...
		//Replace here
		boolean anyNumberGreaterThan4 = numbers.stream().anyMatch(i -> i>4);
	...
	public void shouldCheckIfEachNumberIsEven() throws Exception {
		...
		//Replace here
		boolean eachNumberIsPair = numbers.stream().allMatch(i -> i%2 ==0);
	...
	public void shouldReduceOnlyPositiveValuesToCalculateMultiplyThem() throws Exception {
		...
		List<BigDecimal> filtredNumbers = numbers.stream()
				.filter(i -> i.compareTo( BigDecimal.ZERO)>0)
				.collect(Collectors.toList());
		Optional<BigDecimal> reduced = filtredNumbers.stream()
